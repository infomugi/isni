...
class KonsultasiController extends Controller
{
	...
	public function actionView($id)
	{
		...
		$jawaban=$this->loadModel($id);
		if(isset($_POST['Konsultasi']))
		{
			$jawaban->attributes=$_POST['Konsultasi'];
			$jawaban->admin_id = YII::app()->user->id;
			$jawaban->tanggal_jawab = date('Y-m-d h:i:s');
			if($jawaban->save()){
				$this->refresh();
			}
		}
	...
	public function actionIndex()
	{
		...
		$dataProvider=new CActiveDataProvider('Konsultasi',
			array(
				'criteria'=>array(
					'condition'=>'admin_id=0',
					'order'=>'tanggal_konsultasi DESC',
					'limit'=>10
					),
				'pagination'=>false,
				));
